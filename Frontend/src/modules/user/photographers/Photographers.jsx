...
const Photographers = () => {
  ...
  const collections = [
    ...
    { 
      id: 'top-rated', 
      name: 'Top Rated', 
      count: allPhotographers.filter(p => p.rating >= 4.7).length,
      icon: 'star',
    ...
    { 
      id: 'trending', 
      name: 'Trending', 
      count: allPhotographers.filter(p => p.isTrending).length,
      icon: 'trending',
    ...
    { 
      ...
      count: allPhotographers.filter(p => {
  ...
  // Filter photographers by collection
  const getCollectionPhotographers = () => {
    switch(selectedCollection) {
      case 'top-rated':
        return allPhotographers.filter(p => p.rating >= 4.7);
      case 'trending':
        return allPhotographers.filter(p => p.isTrending);
      case 'value':
  ...
  const filteredPhotographers = getCollectionPhotographers().filter(photographer => {
    // Search filter
    ...
    const matchesPriceRange = filters.priceRange === 'all' || (() => {
      const price = photographer.price || photographer.priceRange;
      if (!price) return true;
      
      const priceNum = parseInt(price.replace(/[^\d]/g, ''));
      switch (filters.priceRange) {
        case 'budget': return priceNum < 30000;
        case 'mid': return priceNum >= 30000 && priceNum < 50000;
        case 'premium': return priceNum >= 50000;
        default: return true;
      }
    })();
    ...
    const matchesRating = filters.rating === 'all' || (() => {
      const rating = photographer.rating || 0;
    ...
    const matchesAvailability = filters.availability === 'all' || 
      (filters.availability === 'available' && photographer.isAvailable !== false);
    
    return matchesSearch && matchesPriceRange && matchesRating && matchesAvailability;
  });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="budget">Under ₹30K</option>
        ...
        >
          <div className="flex items-center justify-between xs:justify-start">
            <span 
              ...
              style={{ color: theme.semantic.text.primary }}
            >
              {sortedPhotographers.length} photographer{sortedPhotographers.length !== 1 ? 's' : ''} found
              {searchQuery && (
        ...
        {sortedPhotographers.length === 0 && (
          ...
          >
            <div className="mb-6 flex justify-center">
              ...
              >
                <Icon name="camera" size="2xl" style={{ color: theme.colors.primary[500] }} />
              </div>
            </div>
            <h3 
