...
const Makeup = () => {
  ...
  const collections = [
    { 
      id: 'all', 
      name: 'All Makeup Artists', 
      count: allMakeupArtists.length,
      icon: 'grid',
    ...
    { 
      id: 'top-rated', 
      name: 'Top Rated', 
      count: allMakeupArtists.filter(p => p.rating >= 4.7).length,
      icon: 'star',
    ...
    { 
      id: 'trending', 
      name: 'Trending', 
      count: allMakeupArtists.filter(p => p.isTrending).length,
      icon: 'trending',
    ...
    { 
      ...
      count: allMakeupArtists.filter(p => 
        p.services?.some(s => s.toLowerCase().includes('airbrush'))
      ).length,
      icon: 'sparkles',
  ...
  // Filter makeup artists by collection
  const getCollectionMakeupArtists = () => {
    switch(selectedCollection) {
      case 'top-rated':
        return allMakeupArtists.filter(p => p.rating >= 4.7);
      case 'trending':
        return allMakeupArtists.filter(p => p.isTrending);
      case 'airbrush':
        return allMakeupArtists.filter(p => 
          p.services?.some(s => s.toLowerCase().includes('airbrush'))
        );
      case 'luxury':
        return allMakeupArtists.filter(p => {
      ...
      default:
        return allMakeupArtists;
    }
  };

  const filteredMakeupArtists = getCollectionMakeupArtists().filter(artist => {
    // Search filter
    ...
    const matchesPriceRange = filters.priceRange === 'all' || (() => {
      const price = artist.price || artist.priceRange;
      if (!price) return true;
      
      const priceNum = parseInt(price.replace(/[^\d]/g, ''));
      switch (filters.priceRange) {
        case 'budget': return priceNum < 15000;
        case 'mid': return priceNum >= 15000 && priceNum < 25000;
        case 'premium': return priceNum >= 25000;
        default: return true;
      }
    })();
    ...
    const matchesAvailability = filters.availability === 'all' || 
      (filters.availability === 'available' && artist.isAvailable !== false);
    
    return matchesSearch && matchesPriceRange && matchesRating && matchesServices && matchesAvailability;
  });

  const sortedMakeupArtists = [...filteredMakeupArtists].sort((a, b) => {
    switch (sortBy) {
      case 'rating':
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="budget">Under ₹15K</option>
                  <option value="mid">₹15K - ₹25K</option>
                  <option value="premium">Above ₹25K</option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="traditional">Traditional Looks</option>
                </select>
              </div>
        ...
        >
          <div className="flex items-center justify-between xs:justify-start">
            <span 
              ...
              style={{ color: theme.semantic.text.primary }}
            >
              {sortedMakeupArtists.length} makeup artist{sortedMakeupArtists.length !== 1 ? 's' : ''} found
              {searchQuery && (
        ...
        {sortedMakeupArtists.length === 0 && (
          ...
          >
            <div className="mb-6 flex justify-center">
              ...
              >
                <Icon name="sparkles" size="2xl" style={{ color: theme.colors.primary[500] }} />
              </div>
            </div>
            <h3 
